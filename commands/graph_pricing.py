...
class Graph_pricing(BaseCommand):
    ...
    async def handle(self, params, params_optional, message, client):
        ...
        embed = discord.Embed(title="21-day pricing graph for {} {}".format(bot.capitalize(), renewal_param.lower()), description="", color=settings.DEFAULT_EMBED_COLOR)
        embed.add_field(name="\u200b", value="> *WTS*", inline=True)
        embed.add_field(name="\u200b", value="> *WTB*", inline=True)
        embed.add_field(name="\u200b", value="\u200b", inline=True)
        embed.add_field(name="21-day average:", value="${}".format(round(wts_average_all, 2)), inline=True)
        embed.add_field(name="21-day average:", value="${}".format(round(wtb_average_all, 2)), inline=True)
        embed.add_field(name="\u200b", value="\u200b", inline=True)
        embed.add_field(name="7-day average:", value="${}".format(round(wts_average_week, 2)), inline=True)
        embed.add_field(name="7-day average:", value="${}".format(round(wtb_average_week, 2)), inline=True)
        embed.add_field(name="\u200b", value="\u200b", inline=True)
