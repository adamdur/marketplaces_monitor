...
class Losers(BaseCommand):
    ...
    async def handle(self, params, params_optional, message, client):
        ...
        demand_data_sorted = sorted(filtered_demand_data.items(), key=keyfunc, reverse=False)
        price_data_sorted = sorted(filtered_price_data.items(), key=keyfunc, reverse=False)
        ...
        embed = discord.Embed(
            title=f"LIST OF TOP {period} LOSERS IN {type.upper()} DEMAND & PRICING",
            description="",
